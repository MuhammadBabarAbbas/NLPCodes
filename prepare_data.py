...
class CornellMovieCorpusProcessor:
    ...
    def prepare_seq2seq_files(self, questions, answers, output_directory='', test_set_size=30000):
        ...
        for i in range(len(questions)):
            if i in test_ids:
                sentiment = use_neural_net.get_sentiment(questions[i])
                sentence = questions[i]+' '+sentiment
                sentence = sentence.replace('\r', '').replace('\n', '')
                test_enc.write(sentence + '\n')
                sentiment = use_neural_net.get_sentiment(answers[i])
                sentence = answers[i]+' '+sentiment
                sentence = sentence.replace('\r', '').replace('\n', '')
                test_dec.write(sentence + '\n')
            else:
                sentiment = use_neural_net.get_sentiment(questions[i])
                sentence = questions[i]+' '+sentiment
                sentence = sentence.replace('\r', '').replace('\n', '')
                train_enc.write(sentence + '\n')
                sentiment = use_neural_net.get_sentiment(answers[i])
                sentence = answers[i]+' '+sentiment
                sentence = sentence.replace('\r', '').replace('\n', '')
                train_dec.write(sentence + '\n')
