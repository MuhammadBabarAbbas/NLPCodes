...
CORNELL_email_CORPUS_ENCODING = 'iso-8859-1'


class CornellMovieCorpusProcessor:

    def __init__(self, data_directory=DEFAULT_DATA_DIRECTORY, lines='email_lines.txt', conversations='email_conversations.txt'):
        self.email_lines_filepath = os.path.join(os.path.abspath(data_directory), lines)
        self.email_conversations_filepath = os.path.join(os.path.abspath(data_directory), conversations)
    ...
    def get_id2line(self):
        ...
        id_index = 0
        text_index = 4
        with open(self.email_lines_filepath, 'r', encoding=CORNELL_email_CORPUS_ENCODING) as f:
            for line in f:
    ...
    def get_conversations(self):
        """
        1. Read from 'email_conversations.txt'
        2. Create a list of [list of line_id's]
        :return: [list of line_id's]
        """
        conversation_ids_index = -1
        conversations = []
        with open(self.email_conversations_filepath, 'r', encoding=CORNELL_email_CORPUS_ENCODING) as f:
            for line in f:
                items = line.split(DELIM)
    ...
    def prepare_seq2seq_files(self, questions, answers, output_directory='', test_set_size=18):
        """
        ...
        train_enc = open(train_enc_filepath, 'a+', encoding='utf8')
        train_dec = open(train_dec_filepath, 'a+', encoding='utf8')
        test_enc = open(test_enc_filepath, 'a+', encoding='utf8')
        test_dec = open(test_dec_filepath, 'a+', encoding='utf8')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-l', '--lines-filename',
                        dest='lines_filename',
                        default='email_lines.txt',
                        help='Path to Cornell Corpus, "email_lines.txt"')
    parser.add_argument('-c', '--conversations-filename',
                        dest='conversations_filename',
                        default='email_conversations.txt',
                        help='Path to Cornell Corpus, "email_conversations.txt"')
